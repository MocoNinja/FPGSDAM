...
public class Buscaminas extends Applet{
	...
	public void init() {
		...
		{
			campo[i / FILAS][i % COLUMNAS] = new Casilla(i / FILAS * Casilla.ANCHO, i % COLUMNAS * Casilla.ALTO, sprites[CASILLA], this);
		}
		setupMinas();
		for (int i = 0; i < FILAS * COLUMNAS; i++)
		{
	...
	{
		/*
		* Otra forma de hacer numeros aleatorios sin repeticion
		* creamos un vector de resultados resultados resultado = new int[DIM];
		* creamos un vector con todas las posibilidades vector = new int[DIM][DIM], que se carga con un for i -> DIM * DIM
		* generemos un valor entre 0 y vector.lenghh
		* ponemos el numero con el indice generado en resultado
		* copiamos el elemento ultimo a ese
		* generamos un valor entre 0 y vector.length - 1
		* ...
		*/
	...
	public boolean victory()
	{
		...
		for (int i = 0; i < FILAS * COLUMNAS; i++)
		{
			 if (campo[i / FILAS][i % COLUMNAS].isDescubierta() || (campo[i / FILAS][i % COLUMNAS].isMina())) aciertos++;
		}
