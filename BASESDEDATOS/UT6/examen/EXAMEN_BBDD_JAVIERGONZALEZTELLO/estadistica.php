...
<html>
	<head>
		...
		{
	location.href="estadistica.php?fec="+valor;
		}
...
<select name="fecha" id="fecha" onchange="cargar(this.value)">
     <option value=''>Elige fecha
<?php
$sql="SELECT date(fecha) as fecha  FROM bitacora GROUP BY date(fecha)";
if(isset($_GET['fec']))
{
	$f = $_GET['fec'];
	$sql1="SELECT * FROM bitacora WHERE DATE(fecha) = '$f'";
}
else
{
...
$registros1=mysql_query($sql1);
while($linea=mysql_fetch_array($registros))
{
	if(isset($_GET['fec']) && ($linea['fecha']==$_GET['fec'])) 
			echo "<option value='$linea[fecha]' selected>$linea[fecha]";
	else
			echo "<option value='$linea[fecha]'>$linea[fecha]";
}
?>
</select>
			...
			<table>
			<thead><tr><td>Usuario</td><td>Operacion</td><td>Fecha</td></tr></thead>
<?php
while($linea=mysql_fetch_array($registros1))
{
  echo"<tr><td>$linea[usuario]</td><td>$linea[operacion]</td><td>$linea[fecha]</td></tr>";
}
?>
	</table>
